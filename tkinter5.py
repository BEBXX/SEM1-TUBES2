import tkinter as tk
from tkinter import ttk
from tkinter import font
from tkinter.constants import FALSE, TRUE
from tkcalendar import Calendar
from tkinter.scrolledtext import ScrolledText
from time import strftime

todos = {}

def detailTodo(cb=None):
    tanggal = str(cal.selection_get())
    win = tk.Toplevel()
    win.wm_title("Detail todo")
    selectedItem = treev.focus()
    selectedIndex = treev.item(selectedItem)['text']
    selectedTodo = todos[tanggal][selectedIndex]
    judul = tk.StringVar(value=selectedTodo['judul'])
    tk.Label(win, text="Tanggal:").grid(row=0, column=0, sticky='N')
    tk.Label(win, text="{} | {}".format(tanggal, selectedTodo['waktu'])).grid(row=0, column=1, sticky='E')
    tk.Label(win, text="Judul:").grid(row=1, column=0, sticky='N')
    tk.Entry(win, state="disabled", textvariable=judul).grid(row=1, column=1, sticky='E')
    tk.Label(win, text="Keterangan:").grid(row=2, column=0, sticky='N')
    keterangan = ScrolledText(win, width=12, height=5)
    keterangan.grid(row=2, column=1, sticky='E')
    keterangan.insert(tk.INSERT, selectedTodo['keterangan'])
    keterangan.configure(state='disabled')

def LoadTodos():
    global todos
    f = open('kalku.dat','r')
    data = f.read()
    f.close()
    todos = eval(data)
    ListTodo()

def SaveTodos():
    f = open('kalku.dat','w')
    f.write(str(todos))
    f.close()

def delTodo():
    tanggal = str(cal.selection_get())
    selectedItem = treev.focus()
    todos[tanggal].pop(treev.item(selectedItem)['text'])
    ListTodo()

def ListTodo(cb=None):
    for i in treev.get_children():
        treev.delete(i)
    tanggal = str(cal.selection_get())
    if tanggal in todos:
        for i in range(len(todos[tanggal])):
            treev.insert("","end", text=i, values=(todos[tanggal][i]['waktu'], todos[tanggal][i]['judul'] ))

def addTodo(win, key, jam, menit, judul, keterangan):
    newTodo = {
        'waktu':'{}:{}'.format(jam.get(), menit.get()),
        'judul': judul.get(),
        'keterangan': keterangan.get('1.0', tk.END)
    }
    if key in todos:
        todos[key].append(newTodo)
    else:
        todos[key] = [newTodo]
    win.destroy()
    ListTodo()

def AddForm():
    win = tk.Toplevel()
    win.wm_title("+")
    jam = tk.IntVar(value=10)
    menit = tk.IntVar(value=30)
    judul = tk.StringVar(value="")
    tk.Label(win, text="Waktu:").grid(row=0, column=0)
    tk.Spinbox(win, from_=0, to=23, textvariable=jam, width=3).grid(row=0, column=1)
    tk.Spinbox(win, from_=0, to=59, textvariable=menit, width=3).grid(row=0, column=2)
    tk.Label(win, text="Judul:").grid(row=1, column=0)
    tk.Entry(win, textvariable=judul).grid(row=1, column=1, columnspan=2)
    tk.Label(win, text="keterangan:").grid(row=2, column=0)
    keterangan = ScrolledText(win, width=12, height=5)
    keterangan.grid(row=2, column=1, columnspan=2, rowspan=4)
    tanggal = str(cal.selection_get())
    tk.Button(win, text="Tambah", command=lambda: addTodo(win, tanggal, jam, menit, judul, keterangan)).grid(row=6, column=0)

def title():
    waktu = strftime('%H:%M')
    tanggal = str(cal.selection_get())
    root.title(tanggal + " | " + waktu + " | KalKu (KalenderKu)")
    root.after(1000, title)

root = tk.Tk()

root.configure(background="#7D84B2")
tk.Label(root, text="\nKALKU\n(KALENDER-KU)\n", font='Corbel 19 bold',fg = "White",background="#7D84B2").grid(row=0, column=0)
tk.Label(root, text="-"*52, font='Arial 15 bold',fg = "White",background="#7D84B2").grid(row=1, column=0)

s = ttk.Style()
s.configure('Treeview',
    background="#D9DBF1",
    foreground="white",
    rowheight=25,
    fieldbackground="#D9DBF1"
    )
s.map('Treeview',
    background = [('selected', '#474954')])

root.title('Calenderku')
root.iconbitmap(r'C:\Users\Ivan\Downloads\cal.ico')

cal = Calendar(root, font='Arial 15 bold', 
                selectmode='day', local='id_ID', 
                cursor='hand2',
                borderwidth=15,
                background = "#8E9DCC",
                foreground = "Black",
                selectbackground = "#474954",
                headersbackground="#D9DBF1",
                normalbackground = "#F9F9ED",
                weekendbackground="#F9F9ED",
                weekendforeground= "Black",
                othermonthwebackground="#CFCFCB",
                othermonthbackground="#CFCFCB",
                showweeknumbers = False
                )

cal.grid(row=2, column=0, sticky='N', rowspan=7)
cal.bind("<<CalendarSelected>>", ListTodo)
tanggal = str(cal.selection_get())

treev = ttk.Treeview(root)
treev.grid(row=9, column=0, sticky='WNE', rowspan=3, columnspan=1)
Scrollbar = tk.Scrollbar(root, orient='vertical', command=treev.yview)
Scrollbar.grid(row=9, column=0, sticky="ENS", rowspan=4)
treev.configure(yscrollcommand=Scrollbar.set)
treev.bind("<Double-1>", detailTodo)
treev['columns'] = ("1","2")
treev['show'] = 'headings'
treev.column("1", width=100)
treev.heading("1", text="JAM")
treev.heading("2", text="JUDUL")

btnAdd = tk.Button(root, text="TAMBAH",font="Corbel 9 bold", fg = "White", bg = "#474954", width=25,height=2,cursor='hand2', command=AddForm)
btnAdd.grid(row=13, column=0, sticky='WS')

btnDel = tk.Button(root, text="HAPUS",font="Corbel 9 bold", fg = "White", bg = "#474954", width=25,height=2,cursor='hand2', command=delTodo)
btnDel.grid(row=14, column=0, sticky='WS')

btnLoad = tk.Button(root, text="LOAD",font="Corbel 9 bold", fg = "White", bg = "#474954", width=25,height=2,cursor='hand2', command=LoadTodos)
btnLoad.grid(row=13, column=0, sticky='ES')

btnSave = tk.Button(root, text="SAVE",font="Corbel 9 bold", fg = "White", bg = "#474954", width=25,height=2,cursor='hand2', command=SaveTodos)
btnSave.grid(row=14, column=0, sticky='ES')
title()
root.mainloop()